...
def process_rasters_custom(
    ...
    参数:
        ...
        raster_names: 栅格文件名列表
        raster_folder: 栅格文件所在文件夹路径
        output_path: 输出文件路径
    ...
    for raster_name in raster_names:
        raster_path = raster_folder / f"{raster_name}.tif"
    ...
    process_args = []
    for raster_name in raster_names:
        raster_path = str(raster_folder / f"{raster_name}.tif")
        stats_methods = custom_stats.get(raster_name, default_stats)
        process_args.append((gdf, raster_path, stats_methods))  # 移除 pbar
...
if __name__ == "__main__":
    ...
    # 生成完整的栅格列表
    raster_names = ['aspect', 'carbonate', 'channelnetworkbaselevel', 'channelnetworkdistance', 'clay_minerals', 'contrast', 
    ...
    stats_config = {
        # 只需要众数的栅格（分类数据）
        'majority': ['dl', 'dz', 'slopepostion'],
    }
    # 将stats_config转换为custom_stats格式
    custom_stats = {}
    stats_methods = {
        'majority': ['majority'],
    }
    ...
    # 根据配置生成custom_stats
    for stats_type, rasters in stats_config.items():
        for raster in rasters:
            custom_stats[raster] = stats_methods[stats_type]
    # 运行处理
    process_rasters_custom(
        ...
        default_stats=['mean'],  # 其他未指定的栅格默认只计算均值
        output_format='csv',
        simplify=True,
