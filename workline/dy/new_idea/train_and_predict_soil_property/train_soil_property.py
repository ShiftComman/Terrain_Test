...
def load_data(file_path,label_col,logger):
    ...
    data = data[(data[f"{label_col}_Sta"]!='Global and Spatial Outlier') & (data[f"{label_col}_Sta"]!='Missing')]
...
if __name__ == "__main__":
    file_path = r"G:\soil_property_result\glx\table\soil_property_point.csv"
    label_cols = ['PH','CEC','OM','TN','TP','TK','TSE','AP','SK','AK','HG','AS2','PB','CD','CR','TRRZ','GZCHD','YXTCHD',]  # 修改为列表
    feature_cols = [ 'analyticalhillshading', 'aspect', 'carbonate', 'channelnetworkbaselevel', 'channelnetworkdistance', 'clay_minerals', 
    'contrast', 'convergenceindex', 'correlation', 'dem', 'dissimilarity', 'dl', 'dz', 'entropy', 'etp22_3', 'etp22_mean', 'evi', 
    'ferrous_minerals', 'homogeneity', 'lsfactor', 'lswi', 'mean', 'mndwi', 'mrrtf', 'mrvbf', 'ndmi', 'ndvi', 'ndwi', 
    'night22_', 'pca1', 'pca2', 'plancurvature', 'pre22_3', 'pre22_mean', 'profilecurvature', 'relativeslopeposition', 'rock_outcrop', 
    'savi', 'secondmoment', 'slope', 'slopepostion', 'tmp22_3',  'tmp22_mean', 'topographicwetnessindex', 
    'totalcatchmentarea', 'valleydepth', 'vari', 'variance']
    coord_cols = ["lon", "lat"]
    param_grid = {
        'n_estimators': np.arange(100, 2000, 100),
        'max_depth': [None] + list(np.arange(10, 200, 20)),
        'min_samples_split': np.arange(2, 20, 2),
        'min_samples_leaf': np.arange(1, 20, 2)
    }
    use_feature_optimization = True
    save_dir = r'G:\soil_property_result\glx\models\soil_property'
    log_file = r'G:\soil_property_result\glx\logs\train_soil_property.log'
    cv = 10
