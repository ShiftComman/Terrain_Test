...
if __name__ == "__main__":
    # 配置参数
    config = {
        "log_file": r"G:\soil_property_result\dys\logs\predict_soil_properties.log", # 日志文件路径
        "model_dir": r"G:\soil_property_result\dys\models\soil_property\models", # 模型文件路径
        "feature_dir": r"G:\tif_features\county_feature\dy", # 特征文件路径
        "output_dir": r"G:\soil_property_result\dys\soil_property_predict", # 输出文件路径
        "training_data_path": r"G:\soil_property_result\dys\table\soil_property_point.csv", # 训练数据路径
        "coord_cols": ["lon", "lat"], # 坐标列
        "use_rk": True, # 是否使用RFRK模型
        "shapefile_path": r"F:\cache_data\shp_file\dy\dy_studyarea.shp", # 掩膜矢量文件路径
